    ...
    <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <div className="flex min-h-[360px] flex-col gap-3 p-5">
          <div className="flex-1 space-y-3 overflow-y-auto">
            {messages.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`flex gap-2 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                ...
                <p className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${message.role === "user" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}>
                  {message.content}
                </p>
          ...
          <form onSubmit={sendMessage} className="flex gap-2 border-t border-slate-200 pt-4 dark:border-slate-700">
            <input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Tulis kendala Anda..." className="min-w-0 flex-1 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:border-blue-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
            <button type="submit" disabled={isLoading || !input.trim()} aria-label="Kirim pesan" className="rounded-xl bg-blue-600 px-4 text-white transition-colors hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"><Send className="h-4 w-4" /></button>
          </form>
        </div>
      </section>
      ...
      <aside className="space-y-6">
        ...
        <section className="rounded-2xl bg-emerald-600 p-5 text-white shadow-sm">
          ...
          <p className="mt-1 text-sm text-emerald-100">Hubungkan percakapan Anda ke CS/admin untuk verifikasi akun dan bantuan manual.</p>
          <a href={`https://wa.me/${CS_WHATSAPP}?text=${encodeURIComponent("Halo CS Balikin, saya butuh bantuan dari Helpdesk.")}`} target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex rounded-xl bg-white px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50">Chat CS/Admin</a>
        </section>
      </aside>
    </div>
  );
