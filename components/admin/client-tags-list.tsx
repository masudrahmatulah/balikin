    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div className="flex w-full items-center gap-2 sm:w-auto">
        <button
          ...
          type="button"
          aria-label={`Generate QR code untuk ${tag.name}`}
        >
          {isGenerating ? (
            <>
...
const QRModal = memo(({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`QR Code untuk ${selectedTag.name}`}>
...
export const ClientTagsList = memo(function ClientTagsList({ tags, clientId }: ClientTagsListProps) {
  const router = useRouter();
  const [selectedTag, setSelectedTag] = useState<Tag | null>(null);
  const [qrDataUrl, setQrDataUrl] = useState("");
  const [isGenerating, setIsGenerating] = useState(false);

  const generateQR = useCallback(async (tag: Tag) => {
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {tags.map((tag) => (
              <TagItem
                key={tag.id}
                tag={tag}
                isGenerating={isGenerating && selectedTag?.id === tag.id}
...
ClientTagsList.displayName = 'ClientTagsList';
