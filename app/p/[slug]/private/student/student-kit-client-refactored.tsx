...
/**
 * Student Kit Client Component - Refactored for Security & Performance
 *
 * IMPROVEMENTS:
 * ✅ XSS prevention via safeJsonParse()
...
export function StudentKitClient({ studentData }: StudentKitClientProps) {
  ...
  // Parse vCard data SAFELY (XSS prevention)
  const vcardData = useMemo(() => {
    if (!studentData?.vcardData) return null;

    try {
      const parsed = safeJsonParse(studentData.vcardData, {});
      ...
      console.error('[Student Kit] Failed to parse vCard data:', error);
      return null;
    }
  }, [studentData?.vcardData]);
  ...
  const handleShare = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Gagal membuat kode berbagi');
      }
      ...
      <div className="flex gap-2 justify-end mb-4">
        ...
        {hasStudentData && (
          <Button
            onClick={handleShare}
            className="flex items-center gap-2"
            disabled={!!shareData} // Prevent double-click
          >
